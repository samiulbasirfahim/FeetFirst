...
export default function Screen() {
    ...
    const { setParam, getParam } = useParamStore();
    ...
    const handleBeforeNavigate = () => {
        setParam("insole_type", null);
        if (winsolePicked) {
            setParam("came_from_scan_upload", "true");
        }
    };

    const defaultValue = useMemo(() => {
        const prev_picked = getParam("insole_type");
        if (prev_picked && prev_picked === "everyday") {
            return isGerman() ? optionsDE[0] : optionsIT[0];
        } else {
            return null;
        }
    }, [getParam]);
